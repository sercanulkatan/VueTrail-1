...
const actions = {
    ...
    async updateAccounts({
        ...
        if (prms.type == 1) { //Satım
            state.Account.Balance = parseFloat(state.Account.Balance) + parseFloat(prms.val);
            state.Account.Sold = parseFloat(state.Account.Sold) + parseFloat(prms.val);
        } else if (prms.type == 2) { //Alım
            state.Account.Balance = parseFloat(state.Account.Balance) - parseFloat(prms.val);
            state.Account.Recieved = parseFloat(state.Account.Recieved) + parseFloat(prms.val);
